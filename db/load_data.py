...
conn = psycopg2.connect(dbname="tuxemon", user="giorgio")
cursor = conn.cursor()


def get_or_create(table, name):
    cursor.execute(
        f"INSERT INTO {table} (name) VALUES (%s) ON CONFLICT (name) DO NOTHING", (name,)
    )
...
def insert_monster(data):
    ...
    """,
        (
            ...
            data["weight"],
            data["catch_rate"],
            data["lower_catch_resistance"],
            data["upper_catch_resistance"],
        ),
    )
    return cursor.fetchone()[0]
...
def insert_many_to_many(table, monster_id, items):
    lookup_tables = {
        "monster_types": "types",
        "monster_tags": "tags",
        "monster_terrains": "terrains",
    }

    join_column = {
        "monster_types": "type_id",
        "monster_tags": "tag_id",
        "monster_terrains": "terrain_id",
    }

    actual_table = lookup_tables[table]
    fk_column = join_column[table]

    for item in items:
        item_id = get_or_create(actual_table, item)
        cursor.execute(
            f"""
            INSERT INTO {table} (monster_id, {fk_column})
...
def insert_evolutions(monster_id, evolutions):
    for evo in evolutions:
        cursor.execute(
            ...
            VALUES (%s, %s, %s)
        """,
            (monster_id, evo.get("at_level", 0), evo["monster_slug"]),
        )
...
for file in os.listdir(DATA_DIR):
    if file.endswith(".json"):
        with open(os.path.join(DATA_DIR, file)) as f:
            ...
            insert_many_to_many(
                "monster_terrains", monster_id, data.get("terrains", [])
            )
